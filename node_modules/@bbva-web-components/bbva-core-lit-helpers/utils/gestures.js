...
const SUPPORTS_PASSIVE = false;
(function() {
  try {
    const opts = Object.defineProperty({}, 'passive', {get() {SUPPORTS_PASSIVE = true;}});
      ...
      for (let i = 0; i < path.length; i += 1) {
        if (path[i].nodeType === Node.ELEMENT_NODE) {
          if (path[i].localName === 'label') {
            ...
            const ownerLabels = matchingLabels(path[i]);
            // check if one of the clicked labels is labelling this element
            for (let j = 0; j < ownerLabels.length; j += 1) {
...
function setupTeardownMouseCanceller(setup) {
  ...
  for (let i = 0, en; i < events.length; i += 1) {
    en = events[i];
    if (setup) {
...
function firstTouchAction(ev) {
  ...
  if (path) {
    for (let i = 0, n; i < path.length; i += 1) {
      n = path[i];
      if (n[TOUCH_ACTION]) {
        ta = n[TOUCH_ACTION];
        break;
...
function _handleNative(ev) {
  ...
  for (let i = 0, r; i < recognizers.length; i += 1) {
    r = recognizers[i];
...
function _add(node, evType, handler) {
  ...
  const {deps} = recognizer;
  const {name} = recognizer;
  let gobj = node[GESTURE_KEY];
  if (!gobj) {
    node[GESTURE_KEY] = gobj = {};
  }
  for (let i = 0, dep, gd; i < deps.length; i += 1) {
    dep = deps[i];
