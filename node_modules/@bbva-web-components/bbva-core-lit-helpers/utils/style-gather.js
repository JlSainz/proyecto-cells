...
import { BbvaCoreDomModule } from '../elements/bbva-core-dom-module.js';
 ...
 * @return {?CellsDomModule} .
 */
function importModule(moduleId) {
  return /** @type {?CellsDomModule} */ (BbvaCoreDomModule.import(moduleId));
}
...
function styleForImport(importDoc) {
  ...
  const container = importDoc.body ? importDoc.body : importDoc;
...
export function stylesFromModule(moduleId) {
  ...
  if (m._styles === undefined) {
    ...
    if (template) {
      styles.push(
        ...stylesFromTemplate(template, /** @type {templateWithAssetPath} */ (m).assetpath),
      );
    }
 ...
 */
export function stylesFromTemplate(tmp, baseURI) {
  const template = tmp;
  if (!template._styles) {
    const styles = [];
    ...
    for (let i = 0; i < e$.length; i += 1) {
      ...
      const include = e.getAttribute(INCLUDE_ATTR);
      if (include) {
        styles.push(
          ...stylesFromModules(include).filter((item, index, self) => self.indexOf(item) === index),
        );
      }
      if (baseURI) {
...
export function cssFromModule(moduleId) {
  ...
  if (m && m._cssText === undefined) {
    ...
    // include css from the first template in the module
    const t = /** @type {?HTMLTemplateElement} */ (m.querySelector('template'));
    if (t) {
...
function _cssFromModuleImports(module) {
  let cssText = '';
  const styles = _stylesFromModuleImports(module);
